...
//	$rj.='"addressee_name":"'.json_encode($tj['addressee_name']).'",';
  ...
  public function actionExcessive(){
    //  验证token
      ...
      $hospital_id = $doctor_is['hospital_id'];
      if($doctor_is['is_dean'] == 1){
          $where = "hospital_id =  '$hospital_id' and excessive_prescription = 0 and is_reconfirmed = 0";
      }else{
          $where = "doctor_id = '$doctor_id' and excessive_prescription = 0 and is_reconfirmed = 0";
      }
      //查询禁忌超量的药方
      $prescription = new TblPrescription();
...
//    }
  ...
  //      $prescription->validate();
       ...
       // }
        ...
        $list['code'] = "200";
        $list['message'] = "处方上传成功";
        $prescription_list=$prescription->find()->select(array('created_at','prescription_id','production_type_name','prescription_status'))->where("prescription_id='$prescription_id'")->asArray()->one();
		...
		$prescription_list['patient_list']['created_at']=$prescription_list['created_at'];
		$prescription_list['patient_list']['prescription_id']=$prescription_list['prescription_id'];
		$prescription_list['patient_list']['production_type_name']=$prescription_list['production_type_name'];
		$prescription_list['patient_list']['prescription_status']=$prescription_list['prescription_status'];
    ...
    public function actionConfirmexcess(){
        ...
        $tbl_prescription -> updateAll(['is_reconfirmed'=>1],['prescription_id'=>$prescription_id]);
        $tbl_prescription_photo =new TblPrescriptionPhoto();
    ...
    public function actionFried(){
        ...
        if($user_token==""){
         ...
         if(empty($boiling_list['staff_id'])){
                $list['code'] = '202';
                $list['message'] = '暂无数据';
               echo json_encode($list);die;     
         }
