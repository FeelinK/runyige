...
public function actionStatistical_data(){
   ...
   $hospital_id=$is_dean['hospital_id'];
    ...
    from tbl_prescription where $where GROUP BY DATE_FORMAT(created_at, '%Y-%m-%d' ) 
    ";
    $db=Yii::$app->db;
...
//					echo $rj;
		  ...
		  //根据doctorid然后查药方表 然后返给对应的数据
            ...
            if(empty($time)) {
                $mrr = $tblprescription->find()->select(array('prescription_id', 'created_at', 'prescription_time', 'prescription_status'))->where("doctor_id='$doctorid' and prescription_status>0")->asarray()->all();
            }else{
                $mrr = $tblprescription->find()->select(array('prescription_id', 'created_at', 'prescription_time', 'prescription_status'))->where("doctor_id='$doctorid' and prescription_status>0 $where")->asarray()->all();
            }
             if(empty($mrr)){
                 $login['code']="201";
                 $login['message']="暂无数据";
