...
define('APPKEY','FjT2TOCi4V53VmnDK3kQX');
define('APPID','YzLHXxopfm7Uo6Q6Ke2Ar9');
define('MASTERSECRET','tWsHOtmF3l8uE4YeTT7LF3');
define('HOST','http://sdk.open.api.igexin.com/apiex.htm');
/*android*/
//define('CID','9e697a6308e8a9379ea8c9e247f3b45b');
/*ios*/
define('CID','33202c171d7664c5e36044e4cdbc7b17');

define('DEVICETOKEN','614718cb9ba45226c43f878f5cd9f3099892c03ae98824d397dcb1f8c6ac9206');
...
function pushAPN(){
	$igt = new IGeTui(HOST,APPKEY,MASTERSECRET);
    	...
    	$template->set_pushInfo("",1, "howdotester", "default", "123456", "", "", "",1);
...
function pushMessageToApp(){
	...
	$rep = $igt->pushMessageToApp($message,'toApptest');//根据TaskId设置组名，支持下划线，中文，英文，数字
...
function IGtLinkTemplateDemo(){
        ...
        $template ->set_appId(APPID);//应用appid
        $template ->set_appkey(APPKEY);//应用appkey
        $template ->set_title("howdo官网");//通知栏标题
        $template ->set_text("点我进入好度官方网站");//通知栏内容
        $template ->set_logo("");//通知栏logo
        $template ->set_isRing(true);//是否响铃
        $template ->set_isVibrate(true);//是否震动
        $template ->set_isClearable(true);//通知栏是否可清除
        $template ->set_url("http://www.howdo.cc");//打开连接地址
...
function IGtNotificationTemplateDemo(){
        ...
        $template->set_transmissionContent("howdo tester");//透传内容
        $template->set_title("howdo学生端");//通知栏标题
        $template->set_text("你的提问收到了一个新的回复");//通知栏内容
        $template->set_logo("http://rmf.howdo.cc/common/admin/images/add_button_b.png");//通知栏logo
        $template->set_isRing(true);//是否响铃
        $template->set_isVibrate(true);//是否震动
        $template->set_isClearable(true);//通知栏是否可清除
        // iOS推送需要设置的pushInfo字段
...
function IGtTransmissionTemplateDemo(){
	...
	//iOS推送需要设置的pushInfo字段
	//$template ->set_pushInfo($actionLocKey,$badge,$message,$sound,$payload,$locKey,$locArgs,$launchImage);
	$template ->set_pushInfo("", +1, "howdo tester", "", "", "", "", "",1);
        return $template;
}
